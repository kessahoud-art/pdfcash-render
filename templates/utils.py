...
CORRIGE : zero rect().fill() — compatibilite Android totale
"""
import re
import unicodedata
import io
...
def tip_box(c, text, y, ac_color, label="CONSEIL"):
    """Encadre conseil — stroke uniquement, zero fill."""
    text = clean(text)
    if not text:
        return y
    tmp = canvas.Canvas(io.BytesIO(), pagesize=A4)
    h = text_height(tmp, text, TW - 24, "Helvetica", 10) + 32
    # Bordure stroke uniquement
    c.setStrokeColor(ac_color)
    c.setLineWidth(1.5)
    ...
    c.setFont("Helvetica-Bold", 8)
    c.setFillColor(ac_color)
    c.drawString(ML + 10, y - 13, label)
    # Texte
    wrap_text(c, text, ML + 10, y - 26, TW - 24, "Helvetica", 10, DARK)
    return y - h - 10
...
def draw_table(c, headers, rows, y, ac_color, col_widths=None):
    """Tableau zero fill — en-tete = lignes epaisses + texte colore."""
    n_cols = len(headers)
    if col_widths is None:
        col_widths = [TW / n_cols] * n_cols
    row_h = 22

    # En-tete : deux lignes epaisses + texte colore
    c.setStrokeColor(ac_color)
    c.setLineWidth(2.5)
    c.line(ML, y, ML + TW, y)
    c.line(ML, y - row_h, ML + TW, y - row_h)
    x = ML
    ...
    # Bordure exterieure stroke uniquement
...
def draw_cta_box(c, text, price, y, ac_color):
    """CTA — stroke uniquement."""
    text = clean(text)
    price = clean(price)
    tmp = canvas.Canvas(io.BytesIO(), pagesize=A4)
    h_text = text_height(tmp, text, TW - 30, "Helvetica-Bold", 12) + 50
    box_h = h_text
    c.setStrokeColor(ac_color)
    c.setLineWidth(2)
...
def metric_boxes(c, items, y, border=BORDER):
    """Boites metriques — stroke uniquement."""
    n = len(items)
    bw = (TW - (n - 1) * 8) / n
    bh = 50
...
def cover_base(c, label, title, subtitle, tagline, price, author, ac_color, infos=""):
    """
    Couverture minimaliste — ZERO rect().fill().
    Bande en haut = ligne epaisse stroke uniquement.
    """
    cx = W / 2

    # Ligne epaisse en haut (remplace rect fill)
    c.setStrokeColor(ac_color)
    c.setLineWidth(6)
    c.line(0, H - 3, W, H - 3)
